import type { TenderizeChains, TenderizeConfig, TenderizeConfigOptions } from '@lib/types';
import { http, createConfig } from 'wagmi'
import { mainnet, arbitrum, type Chain } from "wagmi/chains";
...
export const createTenderizeConfig: CreateTenderizeConfig = (config) => {
    if (!Object.keys(config.tenderizers === Object.keys(config.chains))) {
        throw new Error("Tenderizers and chains configuration must match supported tokens")
    }
